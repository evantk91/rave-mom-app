...
`login.html` holds three views — welcome, login, signup — but only one shows: `js/login.js` toggles `login-view` and `signup-view` classes on `<body>` and `css/login.css` decides what each means — the same split as `game-over` between `js/gamescene.js` and `css/game.css`. **Welcome is the absence of both classes**, so the default view comes from the markup and the stylesheet rather than from the script having run. If `js/login.js` fails to load you get the splash on screen with inert buttons, rather than a card with everything hidden and no way to reveal any of it — logging in or signing up still needs the script regardless, but the failure mode is a page that doesn't respond instead of one with nothing on it.

The welcome view is a pitch, not a form: the game's own title-screen art (`sprite_sheets/png_sheets/start_menu.png`, the sheet `js/startmenu.js` animates in-game) reused unanimated as static hero art via `.hero-logo` in `css/login.css`. It's a 16-frame sprite strip, so showing frame 0 alone and cropping out the game's "click to start"/attribution lines beneath it is done entirely with `background-position`/`background-size`/`aspect-ratio` — no image was re-exported or cropped on disk. `background-size` is expressed in percent rather than px specifically so it scales down with the fluid card on mobile; see the comment on `.hero-logo` for the math. A **Register** button (`#register-cta`, reusing the shared `.button` look every other page uses for its primary action) goes to signup; a smaller "Already have an account? Log in" line, styled like the forms' own switch links, goes to login. Both login and signup carry their own small "Back" button (`.back-link`) back to welcome, so following either path by mistake doesn't strand the player — the forms' mutual "Sign up"/"Log in" switch links still go directly between each other without a trip through welcome.

Two things to leave alone:
- **The forms stay two separate `<form>` elements.** A form only validates its own controls, so the hidden form's `required` and `minlength` can never block the visible form's submit. Merging them into one form is what would create that problem — and the browser can't focus a `display: none` field to report the error, so it presents as a dead button with no message.
